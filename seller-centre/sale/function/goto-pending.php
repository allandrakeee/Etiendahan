<?php  
	...
	$orders_unique_hash = $mysqli->query("SELECT * FROM tbl_orders WHERE unique_hash_id = '$unique_hash_id' AND status = 'shipped'");
	while($orders_unique_hash_row = mysqli_fetch_assoc($orders_unique_hash)):
		...
		while($orders_product_row = mysqli_fetch_assoc($orders_product_result)):
			...
			$sql = "UPDATE tbl_orders SET status = 'processing' WHERE unique_hash_id = '$unique_hash_id' AND product_id = '$orders_product_id'";
			$mysqli->query($sql);
		endwhile;
	endwhile;
